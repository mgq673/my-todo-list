...
/* harmony import */ var _models_task__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../models/task */ "./src/app/todo/models/task.ts");
...
var TaskService = /** @class */ (function () {
    function TaskService(http, store) {
        ...
        this.uriBase = "" + _environments_environment__WEBPACK_IMPORTED_MODULE_7__["environment"].apiUrl;
        this.params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpParams"]();
        this.loggedInUserId = this.store.pipe(Object(_ngrx_store__WEBPACK_IMPORTED_MODULE_3__["select"])(_store_to_do_reducer__WEBPACK_IMPORTED_MODULE_10__["getUserId"]))
            .subscribe(function (userId) {
            _this.loggedInUserId = userId;
        });
    }
    TaskService.prototype.getTasks = function (list) {
        console.log(JSON.stringify(this.loggedInUserId));
        return this.http
            // .get(`${this.uriBase}${this.loggedInUserId}/lists/${list.id}/tasks`)
            .get(this.uriBase + "/lists/" + list.id + "/tasks", { params: { id: this.loggedInUserId } })
            .map(function (response) {
            var tasks = response['result'].tasks;
            return tasks.map(function (task) { return new _models_task__WEBPACK_IMPORTED_MODULE_8__["Task"]({ id: task._id, listId: task.listid, title: task.title, status: task.status }); });
        });
    };
    TaskService.prototype.addTask = function (payload) {
        var userId = payload.userId, list = payload.list, title = payload.title;
        return this.http
            .post(this.uriBase + "/lists/" + list.id + "/tasks", { title: title, userId: this.loggedInUserId }, this.httpOptions);
    };
    ...
    TaskService.prototype.removeManyTasks = function (payload) {
        ...
        return this.http
            .delete(this.uriBase + "/lists/" + list.id + "/tasks", options);
    };
...
module.exports = __webpack_require__(/*! D:\Programing\LearningPrograming\intelEx\todolist\src\main.ts */"./src/main.ts");
